...
void Player::updatePosition()
{
    ...
    x_pos += x_vel;
	if (x_pos  < 0 || (x_pos  + 100 > 1280))//100 = tile size, 1280 = screen width
			x_pos  -= x_vel;
	//move vertically, contrained by depth of world
    y_pos += y_vel;
	if (y_pos < 0 || (y_pos + 100 > 3000)) //100 = tile size, 3000 = world depth 
			y_pos -= y_vel;
}

//probably will pass in array of rects that will s force
